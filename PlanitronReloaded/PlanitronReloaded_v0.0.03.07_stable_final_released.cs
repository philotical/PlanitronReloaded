...
namespace Philotical
{
    ...
    {
        string path_backup_cfg = "GameData/Philotical/Planitron/default_planet_backup_file.cfg";
        ...
        GUIStyle Planitron_sty;
        GUIStyle Planitron_sty_selected;


        ConfigNode backup_root=null;

        private void backup_GUIwrapper()
        {
            if (backup_exists())
            { // show restore button
            ...
            { // ask if backup shall be made now
                display_backup_info();
            }
        }
        private void display_backup_info()
        { // show the info and the "create restore point now" button
            GUILayout.BeginHorizontal();
            GUILayout.Label("You have no Restore point Defined!");
            if (GUILayout.Button("Create a Restore Point at " + _currentPlanetName, Planitron_sty))
            {
                backup_create(_currentPlanetName);
            }

            GUILayout.EndHorizontal();
        }
        private void display_restore_info()
        { // show the info and the "restore defaults" button
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Restore Defaults", Planitron_sty))
            {
                backup_restore(vessel.mainBody);
            }
            if (GUILayout.Button("Overwrite Restore Point", Planitron_sty))
            {
                backup_create(vessel.mainBody.GetName());
            }
            GUILayout.EndHorizontal();
        }
        private bool backup_exists() 
        {
            backup_root = backup_load();
            if (backup_root == null)
            {
                return false;
            }
        ...
        private ConfigNode backup_load()
        {
            backup_root = ConfigNode.Load(path_backup_cfg);
            return backup_root;

        }
        private bool backup_create(string _planetName)
        {
            foreach (CelestialBody c in FlightGlobals.Bodies)
            {
                if (_planetName == c.GetName())
                {
                    backup_root = new ConfigNode();
                    ConfigNode RestorePoint = backup_root.AddNode("RestorePoint");
                    RestorePoint.AddValue("PlanetName", c.GetName());
                    RestorePoint.AddValue("atmoshpereTemperatureMultiplier", c.atmoshpereTemperatureMultiplier.ToString());
                    RestorePoint.AddValue("atmosphere", c.atmosphere);
                    RestorePoint.AddValue("atmosphereContainsOxygen", c.atmosphereContainsOxygen);
                    RestorePoint.AddValue("atmosphereMultiplier", c.atmosphereMultiplier.ToString());
                    RestorePoint.AddValue("atmosphereScaleHeight", c.atmosphereScaleHeight.ToString());

                    RestorePoint.AddValue("atmosphericAmbientColor", ConfigNode.WriteColor(c.atmosphericAmbientColor));
                    
                    RestorePoint.AddValue("GeeASL", c.GeeASL.ToString());
                    RestorePoint.AddValue("gMagnitudeAtCenter", c.gMagnitudeAtCenter.ToString());
                    RestorePoint.AddValue("gravParameter", c.gravParameter.ToString());
                    RestorePoint.AddValue("Mass", c.Mass.ToString());
                    RestorePoint.AddValue("pressureMultiplier", c.pressureMultiplier.ToString());
                    RestorePoint.AddValue("staticPressureASL", c.staticPressureASL.ToString());
                    Debug.Log("Planitron: backup_create backup_root=" + backup_root);
                    continue;
                }
            }

            if (!backup_root.Save(path_backup_cfg))
            {
                return false;
            }
            return true;
        }
        private void backup_restore(CelestialBody oldB)
        {
            ConfigNode RestorePoint = backup_root.GetNode("RestorePoint");
            _currentPlanetName = RestorePoint.GetValue("PlanetName");
            RoundedGeeASL = Convert.ToSingle(float.Parse(RestorePoint.GetValue("GeeASL")));
            RoundedGeeASL = Math.Round(RoundedGeeASL, 2);
            _currentGee = RoundedGeeASL.ToString();
            if (RestorePoint.GetValue("atmosphere") == "True")
            {
                _currentatmosphereMultiplier = RestorePoint.GetValue("atmosphereMultiplier");
            }
            else
            {
                _currentatmosphereMultiplier = "None";
            }
            _currentOxygen = (RestorePoint.GetValue("atmosphereContainsOxygen") == "True") ? true : false;
            oldB.atmoshpereTemperatureMultiplier = Convert.ToSingle(float.Parse(RestorePoint.GetValue("atmoshpereTemperatureMultiplier")));
            oldB.atmosphere = RestorePoint.GetValue("atmosphere") == "True" ? true : false;
            oldB.atmosphereContainsOxygen = (RestorePoint.GetValue("atmosphereContainsOxygen") == "True") ? true : false;
            oldB.atmosphereMultiplier = float.Parse(RestorePoint.GetValue("atmosphereMultiplier"));
            oldB.atmosphereScaleHeight = Convert.ToSingle(RestorePoint.GetValue("atmosphereScaleHeight"));
            oldB.atmosphericAmbientColor = ConfigNode.ParseColor(RestorePoint.GetValue("atmosphericAmbientColor"));

            oldB.GeeASL = float.Parse(RestorePoint.GetValue("GeeASL"));
            oldB.gMagnitudeAtCenter = float.Parse(RestorePoint.GetValue("gMagnitudeAtCenter"));
            oldB.gravParameter = float.Parse(RestorePoint.GetValue("gravParameter"));
            oldB.Mass = float.Parse(RestorePoint.GetValue("Mass"));
            oldB.pressureMultiplier = float.Parse(RestorePoint.GetValue("pressureMultiplier"));
            oldB.staticPressureASL = float.Parse(RestorePoint.GetValue("staticPressureASL"));
        ...
        public void PopulateConfigFile()
        {
            ...
            foreach (CelestialBody c in FlightGlobals.Bodies)
            {
                ...
                {
                    Debug.Log("Planitron: PopulateConfigFile Active Default Body on Startup is: c.GetName()=" + c.GetName());
                    Debug.Log("Planitron: PopulateConfigFile vessel.mainBody.GetName()" + vessel.mainBody.GetName());
        ...
        {
            Planitron_sty = new GUIStyle(GUI.skin.button);
            ...
            Planitron_sty_selected = new GUIStyle(GUI.skin.button);
        ...
        private void ModifyPlanet(CelestialBody oldB, CelestialBody newB)
        {
            ...
            _currentOxygen = newB.atmosphereContainsOxygen;
                ...
                //oldB.Radius = newB.Radius;
                //oldB.rotation = newB.rotation;
